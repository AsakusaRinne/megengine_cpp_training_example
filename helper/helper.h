...
#define DTYPE_ASKING_INFO(_name)                                               \
  std::cout << as_integer(DTypeEnum::_name) << " : " << #_name << std::endl;

inline void print_dtype_asking_info() {
  MEGDNN_FOREACH_DTYPE_NAME(DTYPE_ASKING_INFO)
}
#undef DTYPE_ASKING_INFO

} // namespace mgb

namespace megdnn {
static inline bool operator==(const TensorShape &a, const TensorShape &b) {
  return a.eq_shape(b);
}
...
static inline std::ostream &operator<<(std::ostream &ostr, const DType &dt) {
  return ostr << dt.name();
}
...
static inline std::ostream &operator<<(std::ostream &ostr, const CompNode &cn) {
...
static inline std::ostream &operator<<(std::ostream &ostr, const VarNode *var) {
  return ostr << "var@" << static_cast<const void *>(var) << "("
              << (var ? var->cname() : "") << ")";
}

static inline std::ostream &operator<<(std::ostream &ostr, SymbolVar var) {
  return ostr << var.node();
}

static inline std::ostream &operator<<(std::ostream &ostr,
                                       OperatorNodeBase *opr) {
  return ostr << ssprintf("opr@%p{id=%zu,type=%s,name=%s}", opr, opr->id(),
...
// __assert_float_equal(const char *expr0, const char *expr1,
//                      const char *expr_maxerr, float v0, float v1, float
//                      maxerr);

// #define MGB_ASSERT_FLOAT_NEAR(v0, v1, maxerr)                                  \
//   ASSERT_PRED_FORMAT3(::mgb::__assert_float_equal, v0, v1, maxerr)

// #define MGB_ASSERT_FLOAT_EQ(v0, v1) MGB_ASSERT_FLOAT_NEAR(v0, v1, 1e-6)

#define MK(name)                                                               \
  {                                                                            \
    dev_##name,                                                                \
        [&](DeviceTensorND &var) { host_##name.copy_from(var).sync(); }        \
  }

// ::testing::AssertionResult
// __assert_tensor_equal(const char *expr0, const char *expr1,
//                       const char *expr_maxerr, const HostTensorND &v0,
...
// #define MGB_ASSERT_TENSOR_EQ(v0, v1) MGB_ASSERT_TENSOR_NEAR(v0, v1, 1e-6)
...
enum class RandomDistribution { GAUSSIAN, UNIFORM, CONSTANT, CONSECUTIVE };

template <class dtype> struct RandomDistributionDTypeDefault;

template <> struct RandomDistributionDTypeDefault<dtype::Float32> {
  static constexpr auto dist = RandomDistribution::GAUSSIAN;
};
template <> struct RandomDistributionDTypeDefault<dtype::Int8> {
  static constexpr auto dist = RandomDistribution::UNIFORM;
};
template <> struct RandomDistributionDTypeDefault<dtype::Uint8> {
  static constexpr auto dist = RandomDistribution::UNIFORM;
};
template <> struct RandomDistributionDTypeDefault<dtype::Int16> {
  static constexpr auto dist = RandomDistribution::UNIFORM;
};
template <> struct RandomDistributionDTypeDefault<dtype::Int32> {
  static constexpr auto dist = RandomDistribution::UNIFORM;
};
template <> struct RandomDistributionDTypeDefault<dtype::Bool> {
  static constexpr auto dist = RandomDistribution::UNIFORM;
};
template <> struct RandomDistributionDTypeDefault<dtype::QuantizedS8> {
  static constexpr auto dist = RandomDistribution::UNIFORM;
};
...
public:
  HostTensorGeneratorBase(uint64_t seed) : m_rng{seed} {}

  virtual ~HostTensorGeneratorBase() = default;

  virtual std::shared_ptr<HostTensorND> operator()(const TensorShape &shape,
                                                   CompNode cn = {}) = 0;
...
template <class dtype> struct UniformRNGDefaultRange;

template <> struct UniformRNGDefaultRange<dtype::Float32> {
  static constexpr dt_float32 LO = -1.732, HI = 1.732;
};

template <> struct UniformRNGDefaultRange<dtype::Int8> {
  static constexpr dt_int8 LO = -127, HI = 127;
};
template <> struct UniformRNGDefaultRange<dtype::Uint8> {
  static constexpr dt_uint8 LO = 0, HI = 255;
};
template <> struct UniformRNGDefaultRange<dtype::Bool> {
  static constexpr dt_bool LO = false, HI = true;
};
template <> struct UniformRNGDefaultRange<dtype::Int16> {
  static constexpr dt_int16 LO = -32767, HI = 32767;
};
template <> struct UniformRNGDefaultRange<dtype::Int32> {
  static constexpr dt_int32 LO = -32768, HI = 32768;
};
template <> struct UniformRNGDefaultRange<dtype::QuantizedS8> {
  static const dt_qint8 LO, HI;
};

template <> struct UniformRNGDefaultRange<dtype::Quantized8Asymm> {
...
public:
  ...
  HostTensorGenerator(ctype lo = UniformRNGDefaultRange<dtype>::LO,
...
public:
  ...
  HostTensorGenerator(ctype default_val)
      : HostTensorGeneratorBase{next_rand_seed()}, m_default_val{default_val} {}
...
public:
  ...
  HostTensorGenerator(ctype val, ctype delta)
      : HostTensorGeneratorBase{next_rand_seed()}, m_val{val}, m_delta{delta} {}
...
public:
  ...
  HostTensorGenerator(ctype lo = UniformRNGDefaultRange<dtype::QuantizedS8>::LO,
                      ctype hi = UniformRNGDefaultRange<dtype::QuantizedS8>::HI,
                      float scale = 1.f, uint64_t seed = next_rand_seed())
      : HostTensorGeneratorBase{seed}, m_scale{scale}, m_lo{lo}, m_hi{hi} {}
...
public:
  ...
  HostTensorGenerator(
      ...
      uint64_t seed = next_rand_seed())
      : HostTensorGeneratorBase{seed}, m_scale{scale},
        m_zero_point(zero_point), m_lo{lo}, m_hi{hi} {}
...
void write_tensor_to_file(const HostTensorND &hv, const char *fname,
                          char mode = 'w');
