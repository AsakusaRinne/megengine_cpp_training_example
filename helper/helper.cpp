...
void mgb::set_rand_seed(uint64_t seed) { return; }
...
HostTensorGenerator<dtype, RandomDistribution::GAUSSIAN>::operator()(
...
template <typename dtype>
std::shared_ptr<HostTensorND>
HostTensorGenerator<dtype, RandomDistribution::UNIFORM>::operator()(
    const TensorShape &shape, CompNode cn) {
  if (!cn.valid())
    cn = CompNode::load("xpu0");
  std::shared_ptr<HostTensorND> ret =
...
template <typename dtype>
std::shared_ptr<HostTensorND>
HostTensorGenerator<dtype, RandomDistribution::CONSTANT>::operator()(
    const TensorShape &shape, CompNode cn) {
  if (!cn.valid())
    cn = CompNode::load("xpu0");
  std::shared_ptr<HostTensorND> ret =
...
template <typename dtype>
std::shared_ptr<HostTensorND>
HostTensorGenerator<dtype, RandomDistribution::CONSECUTIVE>::operator()(
    const TensorShape &shape, CompNode cn) {
  if (!cn.valid())
    cn = CompNode::load("xpu0");
  std::shared_ptr<HostTensorND> ret =
...
namespace mgb {
template class HostTensorGenerator<dtype::Float32,
                                   RandomDistribution::GAUSSIAN>;
template class HostTensorGenerator<dtype::Float32, RandomDistribution::UNIFORM>;
template class HostTensorGenerator<dtype::Float32,
                                   RandomDistribution::CONSTANT>;
template class HostTensorGenerator<dtype::Float32,
                                   RandomDistribution::CONSECUTIVE>;
template class HostTensorGenerator<dtype::Float16,
                                   RandomDistribution::GAUSSIAN>;
template class HostTensorGenerator<dtype::Int8, RandomDistribution::UNIFORM>;
template class HostTensorGenerator<dtype::Int8, RandomDistribution::CONSTANT>;
template class HostTensorGenerator<dtype::Int8,
                                   RandomDistribution::CONSECUTIVE>;
template class HostTensorGenerator<dtype::Uint8, RandomDistribution::UNIFORM>;
template class HostTensorGenerator<dtype::Uint8, RandomDistribution::CONSTANT>;
template class HostTensorGenerator<dtype::Int16, RandomDistribution::UNIFORM>;
template class HostTensorGenerator<dtype::Int16, RandomDistribution::CONSTANT>;
template class HostTensorGenerator<dtype::Int32, RandomDistribution::UNIFORM>;
template class HostTensorGenerator<dtype::Int32, RandomDistribution::CONSTANT>;
std::shared_ptr<HostTensorND>
HostTensorGenerator<dtype::Bool, RandomDistribution::UNIFORM>::operator()(
    const TensorShape &shape, CompNode cn) {
  if (!cn.valid())
    cn = CompNode::load("xpu0");
...
operator()(const TensorShape &shape, CompNode cn) {
  ...
  double scale =
      (param.dequantize(m_hi) - param.dequantize(m_lo)) / (m_rng.max() + 1.0);
  for (size_t i = 0, it = shape.total_nr_elems(); i < it; ++i) {
...
// mgb::__assert_float_equal(const char *expr0, const char *expr1,
//                           const char * /*expr_maxerr*/, float v0, float v1,
//                           float maxerr) {
//   float err = fabs(v0 - v1) / std::max<float>(1, std::min(fabs(v0),
//   fabs(v1))); if (std::isfinite(v0) && std::isfinite(v1) && err < maxerr) {
//     return ::testing::AssertionSuccess();
//   }
//   return ::testing::AssertionFailure() << ssprintf("Value of: %s\n"
//                                                    "  Actual: %.6g\n"
//                                                    "Expected: %s\n"
//                                                    "Which is: %.6g\n"
//                                                    "   Error: %.4e",
...
// ::testing::AssertionResult
// mgb::__assert_tensor_equal(const char *expr0, const char *expr1,
//                            const char * /*expr_maxerr*/, const HostTensorND
//                            &v0, const HostTensorND &v1, float maxerr) {
...
namespace {
auto mkdir(const char *path, int) { return _mkdir(path); }
...
NamedTemporaryFile::NamedTemporaryFile() {
  ...
  m_fd = mkstemp(name);
  mgb_throw_if(m_fd == -1, MegBrainError, "failed to open temp file `%s': %m",
               name);
  m_fpath = name;
...
bool mgb::check_cambricon_device_available(size_t num) {
  if (CompNode::get_device_count(CompNode::DeviceType::CAMBRICON) < num) {
    mgb_log_warn("skip test case that requires %zu cambricon device(s)", num);
    return false;
  }
  return true;
}
...
bool mgb::check_device_type_avaiable(CompNode::DeviceType device_type) {
  ...
  // BEGIN-INTERNAL
  // case mgb::CompNode::DeviceType::FPGA:
  // case mgb::CompNode::DeviceType::OPENCL:
  // case mgb::CompNode::DeviceType::HEXAGON:
  // case mgb::CompNode::DeviceType::CADENCE:
  case mgb::CompNode::DeviceType::ROCM:
  // END-INTERNAL
...
bool mgb::check_compute_capability(int major, int minor) {
  ...
  bool available =
      prop.major > major || (prop.major == major && prop.minor >= minor);
  if (!available) {
    mgb_log_warn("This testcase is ignored due to insufficient cuda cap(got: "
                 "%d.%d, "
                 "expected: %d.%d)",
...
void mgb::write_tensor_to_file(const HostTensorND &hv, const char *fname,
                               char mode) {
